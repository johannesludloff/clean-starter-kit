...
description: Create applications that integrate with Lujo using OAuth 2.0.
section: developer
order: 2
---

Build applications that let users connect their Lujo accounts. OAuth apps can access financial data on behalf of users, enabling integrations, automations, and custom tools.

## What you can build

With OAuth, you can build:

- **Browser extensions** that display Lujo data
- **Mobile apps** with Lujo integration
...
- **Public apps** listed in the Lujo app directory

## How OAuth works

OAuth 2.0 lets your app request access to a user's Lujo data without handling their password:

1. Your app redirects users to Lujo's authorization page
2. Users log in and approve the requested permissions
3. Lujo redirects back with an authorization code
4. Your app exchanges the code for access tokens
5. Use access tokens to call the Lujo API
...
```
https://app.lujo.app/oauth/authorize?
  response_type=code&
...
After the user authorizes, Lujo redirects to your `redirect_uri`:
...
```bash
curl -X POST https://api.lujo.app/v1/oauth/token \
...
```typescript
import { Lujo } from "@midday-ai/sdk";
...
```bash
curl -X POST https://api.lujo.app/v1/oauth/revoke \
  -H "Content-Type: application/json" \
  -d '{
    "token": "mid_at_xxxxx",
    "client_id": "YOUR_CLIENT_ID",
...
app.get("/connect", (req, res) => {
  ...
  const authUrl = new URL("https://app.lujo.app/oauth/authorize");
...
app.get("/callback", async (req, res) => {
  ...
  const response = await fetch("https://api.lujo.app/v1/oauth/token", {
    method: "POST",
